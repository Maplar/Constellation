# Constellation 3.0：最终确认方案

> 版本定位：受控生态与自动化版  
> 核心扩展：Stable API → Permission Model → Audit / Rollback → Local Plugins → AI Agent → Automation  
> 当前状态：方案层面已确认，必须等 1.0 和 2.0 服务边界稳定后再实现  
> 原项目关系：3.0 不能继承 floral-notepaper 的直接命令暴露方式，必须建立权限、审计、回滚和插件 API 层

---

## 1. 最终结论

Constellation 3.0 的目标不是继续堆功能，而是在 1.0 和 2.0 稳定后，把 Constellation 扩展为可生态化、可自动化、可组合的知识工作平台。
...
> Constellation 3.0 是“受控自动化版”：在稳定 API、权限模型、审计记录和回滚机制之上，开放插件、AI Agent 和自动化工作流。

---

## 2. 前置条件

3.0 不能在 1.0 / 2.0 之前实现。
...
如果这些服务仍在频繁变化，不应开放插件系统和 AI Agent。
...
```text
1.0 提供真实 Markdown 文件、DocumentService、SearchService、SuggestionService 和用户确认写入。
2.0 提供 GraphService、KnowledgeTreeService、CanvasService、DashboardService、VectorService 和 SourceService。
3.0 提供 PluginService、AutomationService、AgentService、AuditService、RollbackService 和 WorkflowService。
```

3.0 不能破坏 1.0 的文件事实源、2.0 的模块边界和用户确认原则。
