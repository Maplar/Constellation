...
- 支持同时存在多个磁贴。
- 支持置顶悬浮。
- 支持贴在桌面模式。该能力需要 Rust 调用 Windows Win32 / WorkerW 相关 API，Tauri 本身只负责窗口外壳。
...
| WorkerW 桌面嵌入 | Rust Win32 封装，迁移旧 WPF 中已经验证过的实现思路 |
...
- [ ] 接入开机自启。
- [ ] 迁移 WorkerW 桌面嵌入能力。
- [ ] 处理 Explorer 重启、多显示器、DPI 缩放等 Windows 边界场景。
...
- [ ] 导出当前笔记。
- [ ] 批量导出。
- [ ] 设置页：笔记目录、快捷键、关闭到托盘、开机自启、默认视图模式。
- [ ] 迁移旧版本数据目录识别策略。
...
3. **保留已验证的 Windows 经验**：热键、托盘、WorkerW 的旧实现是迁移参考。
...
| WorkerW 实现边界 | 待验证 | 需要 Tauri 窗口句柄和 Win32 SetParent 方案验证 |
